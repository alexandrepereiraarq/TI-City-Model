...
;; 21.a renamed the global attractive-2015 to attractive
;; 21.b renamed the global bu2015 to bu
;; 23. added a water parameter to the inputs
;; 23.a added water weight parameters to LI, MI, and HI agents
;; 23.b added a load-water function, water-dateset global, and water patch-own
...
;; 27. created a new agent class: deprived income. created its associates variables (dpu-suburb dpu-neigbhour dpu-bu dpu-cbd dpu-mall dpu-markets dpu-road dpu-density dpu-schools dpu-attractive dpu-health dpu-random dpu-water dpu-maxof) and functions () [TO COMPLETE]
;; 28. created a scenario selector to implement the shared socioeconomic pathways (SSPs, see)
;; 28.a created separate parameterisation for the exclusion layer, with the necessary raster dataset loading functions
;; 28.b created new parameterisation for each scenario [TBD]
;; 29. add developer agents weights to the the set-weights function [TBD]
...
to set-weights
  ...
  ;;[ 6 7 7 4 7 7 6 6 4 7 4 1 5 ] ;; APS: 17.12.2024
  ...
  ;;[ 5 7 2 7 9 6 5 1 4 10 3 1 8 ] ;; APS: 17.12.2024
...
;; land ssp scenario urban growth prediction as limits to available urbanisation
to load-ssp1
...
to load-UDP
  ask patches [
    set UDP (((1 - suburb) * dpu-suburb) + (nom-neighbourhood * dpu-neigbhour) + ((1 - bu) * dpu-bu) +
      ((1 - cbd) * dpu-cbd) + ((1 - shoppingmall) * dpu-mall) + ((1 - markets) * dpu-markets) +
      ((1 - road) * dpu-road) + (density * dpu-density) + ((1 - schools) * dpu-schools) +
      ((1 - attractive) * dpu-attractive) + ((1 - healthfacilities) * dpu-health ) + ((random-float 0.1) * dpu-random) +
      ((1 - water) * dpu-water ) ) ;; APS: added a water parameter to the utility functions
  ]
end
...
to load-ULI
  ask patches [
    set ULI (((1 - suburb) * liu-suburb) + (nom-neighbourhood * liu-neigbhour) + ((1 - bu) * liu-bu) +
      ...
      ((1 - water) * liu-water ) ) ;; APS: added a water parameter to the utility functions
  ]
end
...
to load-UMI
  ask patches [
    set UMI (((1 - suburb) * miu-suburb) + (nom-neighbourhood * miu-neigbhour) +  ((1 - bu) * miu-bu) +
      ...
      ((1 - water) * miu-water)) ;; APS: added a water parameter to the utility functions
    ...
    ;;let max_UMI max [UMI_abs] of patches
    ;;let min_UMI min [UMI_abs] of patches
    ;;let normalised_UMI ( UMI_abs - min_UMI ) / ( max_UMI - min_UMI )
...
to load-UHI
  ask patches [
    set UHI (((1 - suburb) * hiu-suburb) + ((1 - nom-neighbourhood) * hiu-neigbhour) + ((1 - bu) * hiu-bu) +
      ...
      ((1 - water) * hiu-water)) ;; APS: added a water parameter to the utility functions
  ]
end
;;.........................................................................
...
to load-UMED
  ask patches [
    set UMED (((1 - road) * 8) + ((1 - suburb) * 7) + ((1 - attractive) * 7) + ((1 - nom-slope) * 7) + ((1 - schools) * 5))
  ]
end
...
to load-UHED
  ask patches [
    set UHED (((1 - road) * 8) + ((1 - suburb) * 6) + ((1 - attractive) * 10) + ((1 - schools) * 5))
  ]
end
;;........................................................................................................................................
...
to make-deprived-household-development
  ...
  ask max-n-of (x * dpu-maxof) d [UDP] [set first-n-UDP 1] ;; APS: test different parameters

  ask n-of x households with [income-status = 4 and DPH-movement = 0][
    move-to one-of patches with
    [((first-n-UDP = 1) and (DP-movement = 0) and (LI-movement = 0) and (HI-movement = 0) and
...
to make-mid-income-household-development
  ...
  ask n-of x households with [income-status = 2 and MIH-movement = 0] [
    ...
    move-to one-of patches with [(first-n-UMI = 1) and (MI-movement = 0) and
