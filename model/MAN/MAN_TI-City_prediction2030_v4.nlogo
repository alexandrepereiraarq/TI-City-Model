...
to load-exclusion
  ...
  ask patches [if exclusion = 1 [ set pcolor grey ]]
  ;;gis:paint exclusion-dataset 0
end
...
to load-urban2000
  ...
  ask patches [ifelse urban = 1 [ set pcolor red ] [set pcolor green]]
end
...
to export-income-status-as-asc [file-name]
  ;; Create a new raster dataset
  ;;let raster gis:make-empty-dataset min-pxcor max-pxcor min-pycor max-pycor

  ;; Assign values to patches based on pcolor
  ask patches [
    let value -9999 ;; Default to NODATA_value
    if pcolor = yellow [set value 1]
    if pcolor = orange [set value 2]
    if pcolor = blue [set value 3]
    if pcolor = brown [set value 4]
    if pcolor = green [set value 0]
    ;;gis:set-dataset-value raster pxcor pycor value
  ]
